...
void transmettreChainePc(uint8_t mots[], uint8_t nCaractere, uint8_t nTransmissions)
{
  ...
  for (uint8_t i = 0; i < nTransmissions; i++)
  {
    ...
    {
      transmissionUSART(mots[j]);
    }
  }
}

void transmettreChainePc2(const char mots[], uint8_t nCaractere, uint8_t nTransmissions)
{
...
{
  eeprom_write_block((const void *)&tableauAEcrire, (void *)adresse, nCaractere);
